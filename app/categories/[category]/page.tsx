    ...
    <div className="min-h-screen bg-background text-on-background lg:flex">
      ...
      <main className="flex-1 mt-[72px] mb-[88px] lg:mt-0 lg:mb-0 lg:ml-64 p-6 md:p-12 w-full max-w-5xl mx-auto">
        ...
        <div className="mb-6 mt-4 lg:mt-8">
          <Link
            href="/categories"
            className="inline-flex items-center gap-2 text-sm font-bold text-primary dark:text-primary-fixed-dim hover:underline"
          >
        ...
        <div className="mb-10 flex items-center gap-4 animate-slide-in">
          <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-sm ${category.badgeBg}`}>
        ...
        {filteredQuizzes.length === 0 ? (
          <div className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-10 text-center shadow-sm animate-pop-in">
            ...
            <Link
              href="/categories"
              className="chunky-btn bg-primary text-on-primary font-display text-xs px-6 py-2.5 rounded-full mt-4 inline-flex items-center gap-2 font-bold"
            >
              Clear Filter
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-6">
            {filteredQuizzes.map((quiz, index) => {
              ...
              return (
                <div
                  key={quiz.id}
                  className="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/30 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md transition-shadow duration-200 animate-pop-in"
                  style={{ animationDelay: `${index * 80}ms` }}
                >
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <span className={`px-2.5 py-0.5 rounded-md font-display text-[10px] font-extrabold uppercase ${diffBadgeClass}`}>
                        {quiz.difficulty}
                      </span>
                      <span className="text-[11px] text-on-surface-variant font-bold bg-surface-container px-2 py-0.5 rounded">
                        {quiz.questions.length} Questions
                      </span>
                      {completed && (
                        <span className="text-[11px] text-on-tertiary-fixed-variant font-bold bg-tertiary-fixed/20 px-2 py-0.5 rounded-md inline-flex items-center gap-0.5 border border-tertiary/10">
                      ...
                      {progress && (
                        <span className="text-[11px] text-on-primary-fixed-variant font-bold bg-primary-fixed/20 px-2 py-0.5 rounded-md inline-flex items-center gap-0.5 border border-primary/10">
                          <span className="material-symbols-outlined text-[13px]" style={{ fontVariationSettings: "'FILL' 1" }}>
                            pending
                          </span>
                  ...
                  <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-3 border-t md:border-t-0 pt-4 md:pt-0 border-outline-variant/20 shrink-0">
                    ...
                    <Link
                      href={`/quiz/${quiz.id}`}
                      className="chunky-btn bg-primary text-on-primary font-display text-sm px-6 py-3 rounded-full flex items-center gap-2 font-bold hover:scale-105 active:scale-95 transition-transform"
                    >
